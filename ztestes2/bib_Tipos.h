			...
			//	if(){		
				while(auxg!=NULL){	
						...
						printf("\ntipo local %d",(*aux).tipono);
							if ((*auxg).tipono ==0){
								printf("\nNome da variavel global %s",(*auxg).simb.name);
							}else 
								printf("\nNome da funcao global %s",(*auxg).funcao.name);
							if ((*auxg).tipono ==0){
								printf("\ncomparando com variavel local %s",(*aux).simb.name);
							}else 
								printf("\ncomparando com funcao local %s",(*aux).funcao.name);
						//if (isequal(id.name,(*aux).simb.name)){
